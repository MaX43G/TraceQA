...
- **三路混合检索**：查询重写、HyDE、图谱(local+global)+向量(多查询)+关键词(hl_keywords)、RRF 融合、ReRead 补全、语义重排（`BAAI/bge-reranker-v2-m3`，可选）
...
- **知识图谱路径可视化**：回答附带的「图谱」按钮，把回答相关的知识图谱实体-关系子图渲染为可视化路径
