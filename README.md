...
服务器对外 **仅开放 80 / 443 / 6116** 四个端口（Caddy 前端 HTTPS 与 MinIO 头像直链）；其余端口在 Docker
容器内开放供微服务间通信，
需在服务器防火墙阻断外网访问。

| 服务                 | 容器内端口 | 宿主端口        | 对外    |
|----------------------|------------|-----------------|---------|
| Caddy（前端 HTTPS）  | 80/443     | 80 / 443        | ✅ 公开 |
...
- **只开放 80/443/6116？** 微服务各端口在容器内开放供内部通信；请在服务器防火墙阻断 6114、6122-6128、6117-6121
  等端口的外网访问。