...
async function uploadAvatar(blob: Blob): Promise<void> {
  ...
  try {
    ...
    if (json.code === 200 && json.data) {
      await auth.fetchMe()
      await message.success('头像已更新')
    } else {
      await message.error(json.msg || '头像上传失败')
    }
  } catch {
    await message.error('头像上传失败')
  }
}
...
async function saveNickname(): Promise<void> {
  ...
  try {
    ...
    await message.error((err as Error).message || '保存失败')
  } finally {
...
async function savePassword(): Promise<void> {
  if (!oldPwd.value || !newPwd.value) {
    await message.warning('请填写原密码与新密码')
    return
  }
  if (newPwd.value.length < 6) {
    await message.warning('新密码至少 6 位')
    return
  }
  ...
  try {
    ...
    if (json.code === 200) {
      await message.success('密码已修改')
      oldPwd.value = ''
      newPwd.value = ''
      confirmPwd.value = ''
    } else {
      await message.error(json.msg || '修改失败')
    }
  } catch {
