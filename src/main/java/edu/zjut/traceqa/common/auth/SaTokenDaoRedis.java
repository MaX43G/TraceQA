 ...
 * 后 Base64 落 Redis，避免引入第三方序列化库。Redis 不可用时自动降级（不抛异常），
 * 保证登录链路可用。</p>
 */
@Slf4j
public class SaTokenDaoRedis implements SaTokenDao {
    ...
    public SaTokenDaoRedis(StringRedisTemplate stringRedisTemplate) {
    ...
    public List<String> searchData(String prefix, String keyword, int start, int size, boolean sortType) {
        // Redis 无内置模糊匹配分页，返回空（sa-token 单端登录场景不依赖此接口）
